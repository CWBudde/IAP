...
type
  ...
  TPortAudioCallbackInformation = record
    TimeInfo: PPaStreamCallbackTimeInfo;
    StatusFlags: TPaStreamCallbackFlags;
  end;

  TPortAudioStreamCallbackEvent = function(Sender: TObject;
    InputBuffers, OutputBuffers: TIAPArrayOfSingleFixedArray;
    FrameCount: NativeUInt; CallbackInfo: TPortAudioCallbackInformation): LongInt of object;
  ...
  private
    ...
    FInputBuffers: TIAPArrayOfSingleFixedArray;
    FOutputBuffers: TIAPArrayOfSingleFixedArray;
...
begin
  if Active then
  begin
    PaError := Pa_IsStreamStopped(FStream);
...
function TPortAudioHost.StreamCallback(Input, Output: Pointer;
  ...
  StatusFlags: TPaStreamCallbackFlags): LongInt;
var
  CallbackInfo: TPortAudioCallbackInformation;
begin
  if Assigned(FOnStreamCallback) then
  begin
    CallbackInfo.TimeInfo := TimeInfo;
    CallbackInfo.StatusFlags := StatusFlags;
    ...
    Result := FOnStreamCallback(Self, FInputBuffers, FOutputBuffers,
      FrameCount, CallbackInfo);
  end
  else
    Result := paContinue;
end;
...
(*
    // eventually ignore older / proprietary APIs
    if HostApi.HostApiTypeId in [paASIO, paMME] then
      Continue;
*)
