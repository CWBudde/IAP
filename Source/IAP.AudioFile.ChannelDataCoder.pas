...
uses
  Classes, IAP.Types, IAP.Math.HalfFloat;

type
  TIAPByteArray = array [0..0] of Byte;
  PIAPByteArray = ^TIAPByteArray;
  TIAPSmallIntArray = array [0..0] of Smallint;
  PIAPSmallIntArray = ^TIAPSmallIntArray;
  ...
  private
    function GetChannelPointer(Index: Integer): PIAPSingleFixedArray;
  protected
    FChannelArray : array of PIAPSingleFixedArray;
    procedure ChannelCountChanged; override;
    procedure ReallocateChannelMemory; override;
    procedure AssignTo(Dest: TPersistent); override;
  public
    destructor Destroy; override;
    property ChannelPointer[Index: Integer]: PIAPSingleFixedArray read GetChannelPointer;
  end;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  private
    function GetChannelPointer(Index: Integer): PIAPDoubleFixedArray;
  protected
    FChannelArray : array of PIAPDoubleFixedArray;
    procedure ChannelCountChanged; override;
    procedure ReallocateChannelMemory; override;
    procedure AssignTo(Dest: TPersistent); override;
  public
    constructor Create; override;
    property ChannelPointer[Index: Integer]: PIAPDoubleFixedArray read GetChannelPointer;
  end;
...
function TCustomChannel32DataCoder.GetChannelPointer(
  Index: Integer): PIAPSingleFixedArray;
begin
 ...
 case SampleSize of
  1: for Sample := 0 to FSampleFrames - 1 do
      for Channel := 0 to FChannelCount - 1
       do PIAPByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel] := 127 + round(FChannelArray[Channel]^[Sample] * FScaleFactor[0]);
  2: for Sample := 0 to FSampleFrames - 1 do
      for Channel := 0 to FChannelCount - 1
       do PIAPSmallIntArray(FBlockBuffer)^[Sample * FChannelCount + Channel] := round(FChannelArray[Channel]^[Sample] * FScaleFactor[0]);
  3: for Channel := 0 to FChannelCount - 1 do
      for Sample := 0 to FSampleFrames - 1 do
       begin
        ...
        Move(DataInt, PIAPByteArray(FBlockBuffer)^[3 * (Sample * FChannelCount + Channel)], 3);
       end;
...
begin
 ...
 case SampleSize of
  1: for Channel := 0 to FChannelCount - 1 do
      for Sample := 0 to FSampleFrames - 1
       do FChannelArray[Channel]^[Sample] := (PIAPByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel] - 127) * FScaleFactor[1];
  2: for Channel := 0 to FChannelCount - 1 do
      for Sample := 0 to FSampleFrames - 1
       do FChannelArray[Channel]^[Sample] := PIAPSmallIntArray(FBlockBuffer)^[Sample * FChannelCount + Channel] * FScaleFactor[1];
  3: for Channel := 0 to FChannelCount - 1 do
      for Sample := 0 to FSampleFrames - 1
       do FChannelArray[Channel]^[Sample] := (ShortInt(PIAPByteArray(FBlockBuffer)^[3 * (Sample * FChannelCount + Channel) + 2]) shl 16 +
                                                       PIAPByteArray(FBlockBuffer)^[3 * (Sample * FChannelCount + Channel) + 1]  shl  8 +
                                                       PIAPByteArray(FBlockBuffer)^[3 * (Sample * FChannelCount + Channel)]) * FScaleFactor[1];
...
begin
 ...
 for Sample := 0 to FSampleFrames - 1 do
  for Channel := 0 to FChannelCount - 1
   do PIAPByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel] := EncodeALaw(FChannelArray[Channel]^[Sample]);
end;
...
begin
 ...
 for Channel := 0 to FChannelCount - 1 do
  for Sample := 0 to FSampleFrames - 1
   do FChannelArray[Channel]^[Sample] := CALawDecompressTable[PIAPByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel]] * CScale;
end;
...
begin
 ...
 for Sample := 0 to FSampleFrames - 1 do
  for Channel := 0 to FChannelCount - 1
   do PIAPByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel] := LinearToMuLawSample(FChannelArray[Channel]^[Sample]);
end;
...
begin
 ...
 for Channel := 0 to FChannelCount - 1 do
  for Sample := 0 to FSampleFrames - 1
   do FChannelArray[Channel]^[Sample] := CMuLawDecompressTable[(PIAPByteArray(FBlockBuffer)^[Sample * FChannelCount + Channel])] * CScale;
end;
...
begin
 ...
 for Channel := 0 to FChannelCount - 1 do
  for Sample := 0 to FSampleFrames - 1
   do PIAPHalfFloatFixedArray(FBlockBuffer)^[Sample * FChannelCount + Channel] := SingleToHalfFloat(FChannelArray[Channel]^[Sample]);
end;
...
begin
 ...
 for Channel := 0 to FChannelCount - 1 do
  for Sample := 0 to FSampleFrames - 1
   do FChannelArray[Channel]^[Sample] := HalfFloatToSingle(PIAPHalfFloatFixedArray(FBlockBuffer)^[Sample * FChannelCount + Channel]);
end;
...
begin
 ...
 for Channel := 0 to FChannelCount - 1 do
  for Sample := 0 to FSampleFrames - 1
   do PIAPSingleFixedArray(FBlockBuffer)^[Sample * FChannelCount + Channel] := FChannelArray[Channel]^[Sample];
end;
...
begin
 ...
 for Channel := 0 to FChannelCount - 1 do
  for Sample := 0 to FSampleFrames - 1
   do FChannelArray[Channel]^[Sample] := PIAPSingleFixedArray(FBlockBuffer)^[Sample * FChannelCount + Channel];
end;
...
begin
 ...
 for Channel := 0 to FChannelCount - 1 do
  for Sample := 0 to FSampleFrames - 1
   do PIAPDoubleFixedArray(FBlockBuffer)^[Sample * FChannelCount + Channel] := FChannelArray[Channel]^[Sample];
end;
...
begin
 ...
 for Channel := 0 to FChannelCount - 1 do
  for Sample := 0 to FSampleFrames - 1
   do FChannelArray[Channel]^[Sample] := PIAPDoubleFixedArray(FBlockBuffer)^[Sample * FChannelCount + Channel];
end;
...
function TCustomChannel64DataCoder.GetChannelPointer(
  Index: Integer): PIAPDoubleFixedArray;
begin
 if (Index >= 0) and (Index < Length(FChannelArray))
  then Result := FChannelArray[Index]
  else raise Exception.CreateFmt('Index out of bounds (%d)', [Index]);
